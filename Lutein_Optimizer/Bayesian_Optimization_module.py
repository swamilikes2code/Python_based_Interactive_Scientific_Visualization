...
# --- vvvvvvvvvvvvvvvv MODIFIED FUNCTION vvvvvvvvvvvvvvvv ---
def reset_experiment():
    ...
    # --- 2. ADDED: Reset the photobioreactor visualization ---
    # This manually mimics the initial state JS callback
...
def process_and_plot_latest_results():
    ...
    # Run a detailed simulation for plotting
    sol = run_final_simulation(best_params)
    C_x0_model, C_N0_model, F_in_model, C_N_in_model, I0_model = temp_Cx0, temp_CN0, temp_Fin, temp_CNin, temp_I0
    
    max_lutein = sol.y[2, -1]
    final_biomass = sol.y[0, -1]
    final_nitrate = sol.y[1, -1] # <-- Added line to get final nitrate for visualization
    ...
    # --- 3. ADDED: Photobioreactor visualization logic ---
    ...
    # The run_final_simulation call was moved up to avoid running it twice
# --- ^^^^^^^^^^^^^^^^ END MODIFIED FUNCTION ^^^^^^^^^^^^^^^^ ---
...
def run_final_simulation(best_params):
    ...
    # --- 4. ADDED: Return the solution object ---
    return sol
# --- ^^^^^^^^^^^^^^^^ END MODIFIED FUNCTION ^^^^^^^^^^^^^^^^ ---
...
# --- 5. ADDED: Photobioreactor Visualization UI and Callbacks ---
...
# --- 6. ADDED: Layout assembly for the visualization panel ---
GLASS_STYLE = {
...
# --- vvvvvvvvvvvvvvvv MODIFIED LAYOUT vvvvvvvvvvvvvvvv ---
results_col = column(
    data_table, 
    results_div, 
    p_conv, 
    p_sim,
    indicator_panel_title, # <-- Added
    indicator_panel      # <-- Added
)
# --- ^^^^^^^^^^^^^^^^ END MODIFIED LAYOUT ^^^^^^^^^^^^^^^^ ---
