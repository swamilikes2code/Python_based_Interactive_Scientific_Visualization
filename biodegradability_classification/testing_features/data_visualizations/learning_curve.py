...
features = df[['MolWt', 'NumValenceElectrons', 'NumRadicalElectrons']] #independent variables / features 
...
train_sizes = np.linspace(0.1, 1.0, 10) # Defines array from 0.1 to 1.0 in 10 evenly spaced out increments. The result is 10 numbers are generated... 0.1, 0.2, 0.3 ...  to 1.0
...
for size in train_sizes: #for each size in the array train_size 
    ...
    model = DecisionTreeClassifier() # Train the model with new subsets
