...
df = dd.read_csv("./data/all.csv")
print(df.head(5))
# df1 = dd.read_csv("./data/option_1.csv", low_memory=False)
# df2 = dd.read_csv("./data/option_2.csv", low_memory=False)
# df3 = dd.read_csv("./data/option_3.csv", low_memory=False)
# df4 = dd.read_csv("./data/option_4.csv", low_memory=False)

# all_df = [df1, df2, df3, df4]

# just holding mandatory cols
# df = df1.iloc[:, :3].compute()

all_df_dims = [(0,3), (3, 213), (214, 2262), (2263, 4311), (4312, 6347)]
...
all_cols = []
j=0
for temp_tuple in all_df_dims[1:]:
    temp_df = df.iloc[:, temp_tuple[0]:temp_tuple[1]].head(5)
    temp_set = df.iloc[:, temp_tuple[0]:temp_tuple[0]+10].head(15)
    if j == 0:
        temp_set = temp_set.round(3)
    j += 1

    temp_source = ColumnDataSource(temp_set)
    all_tab_source.append(temp_source)

    temp_dict = list(temp_df.map_partitions(lambda x:x.to_dict("list")))
    temp_dict = temp_dict[0]
    temp_cols = [key for key in temp_dict.keys() if key not in mandatory_columns]
    all_cols.append(temp_cols)

# Create figure
data_tab_columns = [TableColumn(field=col, title=col, width=150) for col in (mandatory_columns+all_cols[0][:7])]
data_tab_table = DataTable(source=all_tab_source[0], columns=data_tab_columns, width=1000, height_policy = 'auto', autosize_mode = "none")
...
def update_table(attr, old, new):
    if data_select.value == 'Molecular Properties':
        cols_to_display = all_cols[0][:7]
        table_source = all_tab_source[0]
    elif data_select.value == 'Morgan Fingerprint':
        cols_to_display = all_cols[1][:7]
        table_source = all_tab_source[1]
    elif data_select.value == 'ECFP2':
        cols_to_display = all_cols[2][:7]
        table_source = all_tab_source[2]
    elif data_select.value == 'Path Fingerprint':
        cols_to_display = all_cols[3][:7]
        table_source = all_tab_source[3]
...
def split_data(train_percentage, val_percentage, test_percentage, data_index):
    ...
    temp_df = all_df[data_index].compute()
